...
@section('title', 'Auftragsbestätigung '.$orderConfirmation->number)
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 mb-10">
        ...
        <div class="text-right">
            <div class="inline-block px-4 py-2 rounded-lg gradient-primary text-white text-lg font-bold uppercase tracking-tight shadow-md">Auftragsbestätigung</div>
            <div class="mt-2 text-gray-600 font-medium">{{ $orderConfirmation->number }}</div>
        </div>
    </div>

    {{-- Empfänger --}}
    <div class="border-l-4 border-emerald-600 bg-gray-50/80 rounded-r-lg pl-5 pr-4 py-4 mb-8">
        <h2 class="text-xs font-semibold uppercase tracking-wider text-emerald-700 mb-2">Auftraggeber</h2>
        <div class="text-gray-700">
            @if($orderConfirmation->user->company)<div class="font-medium text-gray-900">{{ $orderConfirmation->user->company }}</div>@endif
            <div>{{ $orderConfirmation->user->name }}</div>
            @if($orderConfirmation->user->street)<div>{{ $orderConfirmation->user->street }}</div>@endif
            @if($orderConfirmation->user->postal_code || $orderConfirmation->user->city)
                <div>{{ trim(($orderConfirmation->user->postal_code ?? '').' '.($orderConfirmation->user->city ?? '')) }}</div>
            @endif
            @if($orderConfirmation->user->country)<div>{{ config('countries.'.$orderConfirmation->user->country, $orderConfirmation->user->country) }}</div>@endif
            <div class="mt-1 text-gray-500 text-xs">{{ $orderConfirmation->user->email }}</div>
        </div>
    </div>

    {{-- Details --}}
    <div class="flex flex-wrap gap-x-8 gap-y-1 mb-8 text-gray-700 text-xs">
        <div><span class="font-medium text-gray-500">Auftragsdatum:</span> {{ $orderConfirmation->order_date->format('d.m.Y') }}</div>
        @if($orderConfirmation->quote)
            <div><span class="font-medium text-gray-500">Angebot:</span> {{ $orderConfirmation->quote->number }}</div>
        @endif
    </div>
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody>
            @foreach($orderConfirmation->lineItems as $item)
    ...
    <div class="flex justify-end mt-6">
        <table class="w-72 border-t-2 border-emerald-600">
            <tr>
                ...
                <td class="py-3 pl-4 text-right font-bold text-emerald-700 tabular-nums text-base">{{ number_format($orderConfirmation->amount, 2, ',', '.') }} €</td>
            </tr>
        </table>
    </div>
